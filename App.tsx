...
import { RefreshCw, Trophy, Users, AlertCircle, Hand, ChevronRight, EyeOff, Eye, User, Bot, ArrowRight, ChevronLeft, Download } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const isBotTurnInSinglePlayer = gameState.gameMode === 'SINGLE_PLAYER' && currentPlayer.isBot;
  ...
  // --- Game Loop: Start Match ---
  const startMatch = (mode: GameMode, playerCount: number = 4) => {
    setSetupPlayerCount(playerCount);
    setGameState(prev => ({
      ...prev,
      gameMode: mode,
      phase: GamePhase.SETUP // Triggers startRound effect
    }));
  };
  ...
  const startRound = useCallback((roundNum: number, existingPlayers?: Player[], mode?: GameMode) => {
    ...
    if (existingPlayers) {
      ...
      if (mode === 'SINGLE_PLAYER') {
        players = [
          { id: 0, name: 'You', isBot: false, hand: [], score: 0, totalScore: 0, lastAction: 'Waiting...' },
          { id: 1, name: 'Bot Alpha', isBot: true, hand: [], score: 0, totalScore: 0, lastAction: 'Waiting...' },
          { id: 2, name: 'Bot Beta', isBot: true, hand: [], score: 0, totalScore: 0, lastAction: 'Waiting...' },
          { id: 3, name: 'Bot Gamma', isBot: true, hand: [], score: 0, totalScore: 0, lastAction: 'Waiting...' }
        ];
      } else {
        // Multiplayer: Dynamic Player Count
        // Use the setupPlayerCount state, defaulting to 4 if something goes wrong
    ...
    setGameState(prev => ({
      ...
      turnLog: [`Round ${roundNum} started! Joker is ${roundJoker.rank}${roundJoker.suit}`],
      winner: null,
      lastDiscardedId: null,
      tossedThisTurn: false,
      pendingDiscard: null
  ...
  }, [gameState.gameMode, setupPlayerCount]);
      ...
      if (gameState.phase === GamePhase.PLAYER_TURN_START) {
        ...
        if (action.type === 'SHOW') {
          ...
          setGameState(prev => ({
            ...
            openDeck: [...prev.openDeck, ...tossedCards],
  ...
  const handleToss = () => {
    ...
    setGameState(prev => ({
      ...
      openDeck: [...prev.openDeck, ...cards], // Tossed cards go to open deck IMMEDIATELY (tossing doesn't block draw)
  ...
  const handleDraw = (source: 'DECK' | 'OPEN') => {
    ...
    if (source === 'OPEN') {
      ...
      if (gameState.phase === GamePhase.PLAYER_DRAW && topCard.id === gameState.lastDiscardedId) {
    ...
    // COMMIT PENDING DISCARD
    // Now that the draw is complete, the card we discarded earlier goes to the Open Pile
  ...
  const finishTurn = (playerId: number, newHand: CardData[], openDeck: CardData[], deck: CardData[], actionLog: string) => {
    ...
    // Turn Logic:
    // If we just finished a Toss Draw (GamePhase.PLAYER_TOSSING_DRAW), the turn ends immediately.
    // If we just finished a Normal Draw (GamePhase.PLAYER_DRAW), the turn ends immediately.
    // In both cases, we move to the next player.
        ...
        <div className="bg-slate-900/50 p-8 rounded-2xl backdrop-blur-sm border border-white/10 shadow-2xl flex flex-col gap-6 w-full max-w-md">
           ...
           {!showMultiplayerSelection ? (
             <>
                ...
                <button 
                  onClick={() => startMatch('SINGLE_PLAYER')}
             ...
             <>
               ...
               <div className="grid grid-cols-2 gap-3 max-h-[40vh] overflow-y-auto">
                 {[2, 3, 4, 5, 6, 7].map(count => (
                   <button 
                     key={count}
                     onClick={() => startMatch('MULTIPLAYER', count)}
  ...
  // View Perspective: Active player is always "hero" at bottom.
  // Others are at top.
    ...
    <div className="h-[100dvh] flex flex-col overflow-hidden bg-[#1a2e1a]">
      ...
      <div className="h-16 bg-[#0f1f0f] flex items-center justify-between px-4 shadow-lg z-20 shrink-0">
        ...
        <div className="flex items-center gap-2">
           <span className="text-xs uppercase tracking-widest text-purple-400 font-bold">Round Joker</span>
           {gameState.roundJoker && (
             <div className="flex bg-white text-black px-2 py-1 rounded border-2 border-purple-500 items-center gap-1">
               ...
               <span className={gameState.roundJoker.suit === '♥' || gameState.roundJoker.suit === '♦' ? 'text-red-600' : 'text-black'}>{gameState.roundJoker.suit}</span>
             </div>
           )}
        </div>
      ...
      <div className="flex-1 relative flex flex-col justify-between p-2 md:p-4 max-w-7xl mx-auto w-full overflow-hidden">
                  ...
                  <div className="absolute top-10 z-30 flex gap-1 bg-black/80 p-1 rounded shadow-2xl animate-in fade-in zoom-in">
                     {opp.hand.map((c, i) => (
                       <Card 
                         ...
                         small 
                         isJoker={gameState.roundJoker ? (c.rank === gameState.roundJoker.rank && c.suit === gameState.roundJoker.suit) : false}
                        />
                     ))}
        ...
        <div className="flex-1 flex items-center justify-center gap-4 md:gap-16 my-2 relative">
            ...
            {/* Middle Area: Open Deck & Pending Discard */}
              ...
              <div className="flex flex-col items-center gap-2 relative">
                 {gameState.openDeck.length > 0 ? (
                   <Card 
                     ...
                     isJoker={gameState.roundJoker && gameState.openDeck[gameState.openDeck.length - 1].rank === gameState.roundJoker.rank && gameState.openDeck[gameState.openDeck.length - 1].suit === gameState.roundJoker.suit}
                ...
                <div className="flex flex-col items-center gap-2 relative animate-in slide-in-from-left-4 fade-in">
                  <div className="relative">
                    <Card 
                      ...
                      isJoker={gameState.roundJoker && gameState.pendingDiscard.rank === gameState.roundJoker.rank && gameState.pendingDiscard.suit === gameState.roundJoker.suit}
        ...
        <div className="flex flex-col items-center gap-4 pb-8 shrink-0">
           ...
           {isPlayerTurn ? (
             ...
             <div className="h-10 flex items-center justify-center text-gray-400 text-sm italic">
                {isBotThinking ? 'Opponent is thinking...' : 'Waiting for opponent...'}
             </div>
           )}

           {/* Current Player Hand (Always shown at bottom) */}
           <div className="flex items-end justify-center -space-x-4 md:-space-x-6 h-[140px] md:h-[160px]">
              {currentPlayer.hand.map((card) => (
                <Card 
                  key={card.id} 
                  // HIDE CARDS IF:
                  // 1. Multiplayer and it's not the active user's turn (e.g. transitioning)
                  // 2. Single Player and it is a Bot's turn (so human doesn't see bot hand)
                  card={(!isPlayerTurn && gameState.gameMode === 'MULTIPLAYER') || isBotTurnInSinglePlayer ? undefined : card} 
                  isJoker={gameState.roundJoker ? (card.rank === gameState.roundJoker.rank && card.suit === gameState.roundJoker.suit) : false}
                  selected={selectedCardIds.includes(card.id)}
                  onClick={() => handleCardClick(card)}
                  className={`transition-all origin-bottom ${isPlayerTurn ? 'hover:z-20' : 'opacity-80'}`}
                />
              ))}
           </div>
           
           <div className="text-xs text-gray-400 mt-2">
              <span className="text-green-400 font-bold uppercase tracking-wider">{currentPlayer.name}</span> • Total Score: <span className="text-white font-bold text-lg">{currentPlayer.totalScore}</span>
           </div>
        </div>
      </div>
